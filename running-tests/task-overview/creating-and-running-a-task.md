...
**Be specific about UI elements.** Name buttons, fields, and screens as they appear in the app. "Tap the Sign In button" is better than "tap the button." If credentials are configured in [Apps → Credentials,](../../apps/credentials.md) you can reference them directly: _"Log in using the Standard User credentials."_
...
6.  **Upload your app or select an existing one.** Click the **upload icon** to open the Upload App modal. Choose:
    ...
    * **iOS IPA** — upload your `.ipa` or `.zip` file (Mac only)
    ...
    If you have already uploaded a build in [Apps → Builds](/broken/pages/yam3tMsYgsIZ4OsfFpQk), select it from the app picker instead of uploading again.
7. **Select a device.** Click the **Devices** picker and choose the device to run on. If no devices are listed, connect one first — see [Devices](/broken/pages/MwTC55dV1ZoHsa3DjrhK).
8. Click **Run**.
...
\
💡 **First run or rerun?** The first time you run a task, Mahoraga reasons through the entire flow. From the second run onwards, it uses a stored path and executes significantly faster. See[ **Rerun** ](rerun.md)for how this works.
...
Every completed task generates an [Execution Report ](/broken/pages/DZuLpK4Xtm4EqYnVfLlE)automatically. The report is available immediately in the **Reports** section.
