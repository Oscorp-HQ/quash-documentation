---
title: "Glossary"
description: "An AI-powered automation module that performs a specific role in the testing workflow. Quash has two agents: Megumi (test generation) and Mahoraga (test execution). They operate sequentially — Reci..."
---
...
One test run on a device — the unit Quash meters usage in. An execution counts wherever it runs (local device, emulator, or Quash-hosted cloud device) and however it is started (the Quash app, the web console, or an AI agent through the MCP server). A test that fails counts the same as one that passes. Generating, refining, and editing tests never counts as an execution.
...
[→ About the Library](/test-management/test-case/about-the-test-cases-library)
...
[→ Configuration](/test-studio/configuration-in-test-studio)
...
[→ What is Test Studio?](/test-studio/overview)
...
→ [Configuration](/test-studio/configuration-in-test-studio) · [Backend Validations](/running-tests/backend-validations)
