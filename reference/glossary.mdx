...
One test run on a device — the unit Quash meters usage in. An execution counts wherever it runs: local device, emulator, or Quash-hosted cloud device. A test that fails counts the same as one that passes. Generating, refining, and editing tests never counts as an execution.
