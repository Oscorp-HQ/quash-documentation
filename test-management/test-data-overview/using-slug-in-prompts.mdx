---
title: "Using Test Data in Prompts"
sidebarTitle: "Using Test Data in Prompts"
description: "Insert slugs in task and recipe prompts so each dataset row drives its own run, with clear placement and optional multi-dataset combinations."
---
...
Slugs work the same way in [Test Studio](/test-studio/overview) recipes. Type `/` in the recipe prompt area — the same dropdown appears. Recipe uses the dataset as context when generating test cases, and Mahoraga uses it during execution.
